""" ARCHIVO SECUNDARIO: Define la interfaz y funcionamiento del registro humanitario."""
...
# Definir: Formto de texto
bold_font = ("Helvetica", 14, "bold")

# CLASE: REGISTRO HUMANITARIO
class RegistroHumanitario:
    ...
    def create_widgets(self):
        ...
        ## Componentes: Texto, cajas de texto, menus de opción multiple, casillas de verificación y botones
        n_rows = 0
    ...
    ## Función: Obtener datos del registro humanitario | Lista
    def getHText(self):
...
def new_entry(workbook, entry, GUI):
    ...
    hum_sheet = workbook.worksheet("Humanitario") # Seleccionar: Hoja de registro humanitario
    cols = len(hum_sheet.row_values(1)) # Obtener: Nombres de columna
    hum_ids = hum_sheet.col_values(1) # Obtener: IDs de registros existentes

    ## Caso: Identificador inexistente en registro GENERAL o nombres mal escritos
    if id not in hum_ids:
        id2 = Global.similar_entries(id, hum_ids[1:], GUI)
        ### Caso: Identificador inexistente en regsitro GENERAL
        if id2 == id:
            #### Error: Se debe realizar el registro GENERAL antes del HUMANITARIO
    ...
    existing_row = hum_sheet.row_values(index)[1:] # Obtener: Registro existnte (ya alamacenado)
    ## Caso: Generar nuevo registro        
    if len(existing_row) == 0:  # Agregar registro Humanitario
        encrypted_entries = [encryption.encrypt_element(e, "H") for e in entry] # Encriptar datos a registrar
        hum_sheet.update('B{}:{}'.format(index, 'BI' + str(index)), [encrypted_entries]) # Insertar: Registros(encrypted entries) en la fila(index) a partir de la columna B del registro humanitario(hum_sheet)
    ## Caso: Registro existente        
        ...
        if ver == 1:
            ...
            hum_sheet.update('B{}:{}'.format(index, 'BI' + str(index)), [values_to_update]) # Actualizar: Registros(values_to_update) en el rango B(index)-AA(index) del regitro humanitario(psi_sheet)
