""" ARCHIVO SECUNDARIO: Define la interfaz y funcionamiento del registro general."""
...
class RegistroGeneral:
    ...
    ## Función: Obtener datos del registro general | Lista
    def getGText(self):
...
def new_entry(workbook, entry, GUI):
    entry = [Transformations.clean_string(s) for s in entry] # Depurar: Datos a registrar
    id = Transformations.generate_id(entry[:3]) # Generar: Identificador
    general_sheet = workbook.worksheet("General") # Seleccionar: Hoja de registro general
    all_entries = general_sheet.col_values(1) # Obtener: IDs de registros existentes
    
    ## Caso: Nuevo registro o registro existente mal escrito(nombres)
    if id not in all_entries:
        ...
        if id2 == id:
            ...
            general_sheet.insert_row(values_to_insert, index+1) # Insertar: Registros(values_to_insert) en la fila(index) del registro general(general_sheet)
            for sheet_name in ["Legal", "Psicosocial", "Humanitario"]: # Insertar: Indices(id) en su respectiva posición en los otros registros
                sheet = workbook.worksheet(sheet_name)
                sheet.update_cell(len(sheet.col_values(1))+1, 1, id)
            return
        ### Caso: El registro existente fue mal escrito(nombres)
    ...
    if ver == 1:
        ...
        #### Caso: Nombres existente != de nombres a registrar
        if Transformations.generate_id(entry[:3]) != current_id:
            ##### Input: Confirmación para actualizar/corregir nombres(e identificador)
        ...
        #### Caso: Nombres existentes = nombres a registrar
        else: ver2 = 0
        ####: Caso: Actualizar registro existente, incluyendo nombres e identificadores
        if ver2 == 1:
            ...
            general_sheet.update('A{}:{}'.format(index, 'K' + str(index)), [values_to_update]) # Actualizar: Registros(values_to_update) en el rango A(index)-K(index) del regitro general(general_sheet)
            for sheet_name in ["Legal", "Psicosocial", "Humanitario"]: # Actualizar: Identificadores en los otros regigstros 
                sheet = workbook.worksheet(sheet_name)
                sheet.update_cell(index, 1, correct_id)
        ####: Caso: Actualizar registro existnte, excluyendo nombres e identificadores
        else:
            entry = entry[3:]
            ...
            general_sheet.update('E{}:{}'.format(index, 'K' + str(index)), [values_to_update]) # Actualizar: Registros(values_to_update) en el rango E(index)-K(index) del registro general(general_sheet)
        return
